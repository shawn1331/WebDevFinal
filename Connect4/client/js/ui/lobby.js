    ...
    if (link) {
      link.addEventListener('click', async (e) => {
        ...
        try {
          ...
          location.href = `play.html?gameId=${res.gameId}&token=${encodeURIComponent(res.guestToken)}`;
        } catch (err) {
...
createForm.addEventListener('submit', async (e) => {
  ...
  try {
    ...
    // host token from API
    location.href = `play.html?gameId=${res.gameId}&token=${encodeURIComponent(res.hostToken)}`;
  } catch (err) {
